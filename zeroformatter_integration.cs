...
builder.Services.AddSingleton<RecyclableMemoryStreamManager>(_ => 
    new RecyclableMemoryStreamManager(
        blockSize: 1.MB,
        largeBufferMultiple: 4.MB,
        maximumBufferSize: 16.MB));

// 2. 零拷贝消息通道（Disruptor模式）
...
builder.Services.AddHostedService<ZeroCopyProcessor>();
...
app.MapGet("/", () => "ZeroFormatter Integration Ready");
app.Run();
...
public sealed class ZeroCopyProcessor : BackgroundService
{
    ...
    [SkipLocalsInit]
    private unsafe void ProcessMessage(MessageEnvelope message, Span<byte> buffer)
    {
        fixed (byte* ptr = buffer)
        {
            // 零拷贝序列化/反序列化
            var serialized = ZeroFormatterSerializer.Serialize(message);
