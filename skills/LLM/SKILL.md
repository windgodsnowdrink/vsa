...
基于 .NET 10 的高性能 LLM 技能，为 .NET 开发者提供强大的 LLM 功能。
...
`csharp
// 注册 LLM 服务
builder.Services.AddSingleton<ILLMProcessor, LLMProcessor>();
...
??  ????? examples.md            # 使用示例
????? scripts/                    # 脚本和工具
    ????? llm_integration.cs     # LLM 核心实现
    ????? llm_integration.run.json  # 运行配置
    ????? llm_integration.setting.json  # 设置文件
`

## 主要功能

1. **模型推理**: 高性能 LLM 模型推理，支持多种模型提供商
2. **语义记忆**: 基于 Semantic Kernel 的语义记忆管理
3. **提示工程**: 优化的提示模板和管理
...
此技能提供完整的 LLM 解决方案，您可以根据需要进行扩展：

1. **自定义实现**: 实现 ILLMService 接口
2. **扩展功能**: 添加新的 LLM 功能
3. **与其他系统集成**: 与其他系统集成
4. **性能优化**: 针对特定场景优化性能
...
- **脚本路径**: scripts/llm_integration.cs
- **运行配置**: scripts/llm_integration.run.json
- **设置文件**: scripts/llm_integration.setting.json

### 执行流程

1. **编译**: 使用 .NET 10 AOT 编译生成单文件可执行程序
2. **部署**: 将生成的可执行文件部署到目标环境
3. **运行**: 执行可执行文件，启动 LLM 服务
4. **集成**: 与主应用集成，使用 LLM 功能
