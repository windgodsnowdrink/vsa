...
builder.Services.AddWebApiClientCore()
    ...
    {
        options.UseProtobufFormatter(new ThreadLocalSpanSerializer(
            new ThreadLocal<Span<byte>>(() => stackalloc byte[1024])));
    });

var app = builder.Build();
app.MapGet("/", () => "Protobuf Integration Ready");
app.Run();

public class ThreadLocalSpanSerializer : IProtoBufSerializer
{
    private readonly ThreadLocal<Span<byte>> _buffer;
    
    public ThreadLocalSpanSerializer(ThreadLocal<Span<byte>> buffer) => _buffer = buffer;
