...
// 状态和触发器枚举定义
public enum WorkflowState { Started, Processing, Completed, Failed }
public enum WorkflowTrigger { Begin, Process, Complete, Fail }

// 状态机配置选项
public class WorkflowStateMachineOptions
{
...
// 状态机接口
public interface IWorkflowStateMachine : IAsyncDisposable
{
    WorkflowState CurrentState { get; }
    Task<bool> CanHandleTriggerAsync(WorkflowTrigger trigger);
    Task FireAsync(WorkflowTrigger trigger, CancellationToken ct = default);
    IAsyncEnumerable<WorkflowState> StateChangesAsync(CancellationToken ct = default);
}

// 状态机实现
public class WorkflowStateMachine : IWorkflowStateMachine
{
    private readonly Channel<WorkflowState> _stateChannel;
    private readonly IStateMachine<WorkflowState, WorkflowTrigger> _stateMachine;
    
    public WorkflowState CurrentState => _stateMachine.CurrentState;

    public WorkflowStateMachine(IOptions<WorkflowStateMachineOptions> options)
    {
        _stateChannel = Channel.CreateUnbounded<WorkflowState>();
        ...
        config.In(WorkflowState.Started)
            ...
            .On(WorkflowTrigger.Fail).TransitionTo(WorkflowState.Failed);
            
        config.In(WorkflowState.Processing)
            .On(WorkflowTrigger.Process).TransitionTo(WorkflowState.Processing)
            .On(WorkflowTrigger.Complete).TransitionTo(WorkflowState.Completed)
            .On(WorkflowTrigger.Fail).TransitionTo(WorkflowState.Failed);
            
        _stateMachine = config.Build(WorkflowState.Started);
    }
    
    public async Task FireAsync(WorkflowTrigger trigger, CancellationToken ct = default)
    {
        await _stateMachine.FireAsync(trigger, ct);
        await _stateChannel.Writer.WriteAsync(_stateMachine.CurrentState, ct);
    }
    
    public IAsyncEnumerable<WorkflowState> StateChangesAsync(CancellationToken ct = default) 
        => _stateChannel.Reader.ReadAllAsync(ct);
    
    public async ValueTask DisposeAsync() => _stateChannel.Writer.Complete();
}

// DI扩展方法
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddWorkflowStateMachine(this IServiceCollection services, 
        Action<WorkflowStateMachineOptions> configure = null)
    {
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        var host = Host.CreateDefaultBuilder(args)
            ...
            {
                services.AddWorkflowStateMachine(options =>
                {
                    options.MaxConcurrentTransitions = 5;
                    options.EnableDiagnostics = true;
                });
                
                services.AddDistributedStateMachine(redisOptions =>
                {
                    redisOptions.SlidingExpiration = TimeSpan.FromHours(1);
                });
            })
            .Build();

        var stateMachine = host.Services.GetRequiredService<IWorkflowStateMachine>();
        
        // 触发状态转换
        await stateMachine.FireAsync(WorkflowTrigger.Start);
        await stateMachine.FireAsync(WorkflowTrigger.Pause);
        await stateMachine.FireAsync(WorkflowTrigger.Resume);
        await stateMachine.FireAsync(WorkflowTrigger.Complete);
