...
[SkipLocalsInit]
public sealed class BatchMiddleware
{
    private readonly Channel<ProxyRequest> _requestChannel;
    private readonly BatchOptions _options;
    private readonly CancellationTokenSource _cts;

    public BatchMiddleware(IOptions<BatchOptions> options)
    {
    ...
    {
        var batch = new List<ProxyRequest>(_options.BatchSize);
        ...
        while (!_cts.IsCancellationRequested)
        {
            ...
            {
                await Task.Delay(_options.BatchTimeout, _cts.Token);
            }
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    private async Task ProcessBatchAsync(IReadOnlyList<ProxyRequest> batch)
    {
        // 实现批量请求处理逻辑
        // ... 实际批处理逻辑实现
    }
