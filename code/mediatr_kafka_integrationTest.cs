...
Console.WriteLine("=== mediatr_kafka_integration.cs Test ===");
...
try
{
    ...
    // 验证 class: KafkaHealthCheck
    var type_KafkaHealthCheck = Type.GetType("KafkaHealthCheck");
    if (type_KafkaHealthCheck != null)
    {
        Console.WriteLine("[PASS] 类型 KafkaHealthCheck (class) 存在");
        var ctors_KafkaHealthCheck = type_KafkaHealthCheck.GetConstructors();
        Console.WriteLine($"[PASS] KafkaHealthCheck 构造函数数量: {ctors_KafkaHealthCheck.Length}");
        var methods_KafkaHealthCheck = type_KafkaHealthCheck.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.DeclaredOnly);
        Console.WriteLine($"[PASS] KafkaHealthCheck 公开方法数量: {methods_KafkaHealthCheck.Length}");
        foreach (var m in methods_KafkaHealthCheck)
            Console.WriteLine($"  [INFO] 方法: {m.Name}");
    }
    else
    {
        Console.WriteLine("[WARN] 类型 KafkaHealthCheck 未找到，尝试无命名空间...");
        type_KafkaHealthCheck = Type.GetType("KafkaHealthCheck");
        if (type_KafkaHealthCheck != null)
            Console.WriteLine("[PASS] 类型 KafkaHealthCheck (无命名空间) 存在");
        else
            Console.WriteLine("[INFO] 类型 KafkaHealthCheck 可能为顶层语句或嵌套类型");
    }
