...
Console.WriteLine("=== quic_production.cs Test ===");

try
{
    // 验证 class: QuicServer
    var type_QuicServer = Type.GetType("QuicServer");
    if (type_QuicServer != null)
    {
        Console.WriteLine("[PASS] 类型 QuicServer (class) 存在");
        var ctors_QuicServer = type_QuicServer.GetConstructors();
        Console.WriteLine($"[PASS] QuicServer 构造函数数量: {ctors_QuicServer.Length}");
        var methods_QuicServer = type_QuicServer.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.DeclaredOnly);
        Console.WriteLine($"[PASS] QuicServer 公开方法数量: {methods_QuicServer.Length}");
        foreach (var m in methods_QuicServer)
            Console.WriteLine($"  [INFO] 方法: {m.Name}");
    }
    else
    {
        Console.WriteLine("[WARN] 类型 QuicServer 未找到，尝试无命名空间...");
        type_QuicServer = Type.GetType("QuicServer");
        if (type_QuicServer != null)
            Console.WriteLine("[PASS] 类型 QuicServer (无命名空间) 存在");
        else
            Console.WriteLine("[INFO] 类型 QuicServer 可能为顶层语句或嵌套类型");
    }
