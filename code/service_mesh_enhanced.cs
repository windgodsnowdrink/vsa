...
builder.Services.AddLinkerdClient(options =>
{
    ...
    options.TLSSettings = new TLSSettings
    {
        CaCertPath = "/var/run/linkerd/tls/ca.crt",
        CertPath = "/var/run/linkerd/tls/cert.pem",
        KeyPath = "/var/run/linkerd/tls/key.pem"
    };
});
...
{
    private readonly ChannelWriter<SslStream> _writer;
    private readonly ThreadLocal<Span<byte>> _buffer;
    
    public ChannelMeshSecurityHandler(Channel<SslStream> channel, ThreadLocal<Span<byte>> buffer)
    {
