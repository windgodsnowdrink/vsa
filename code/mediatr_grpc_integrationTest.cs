...
Console.WriteLine("=== mediatr_grpc_integration.cs Test ===");
...
try
{
    ...
    // 验证 class: MediatRGatewayService
    var type_MediatRGatewayService = Type.GetType("MediatRGatewayService");
    if (type_MediatRGatewayService != null)
    {
        Console.WriteLine("[PASS] 类型 MediatRGatewayService (class) 存在");
        var ctors_MediatRGatewayService = type_MediatRGatewayService.GetConstructors();
        Console.WriteLine($"[PASS] MediatRGatewayService 构造函数数量: {ctors_MediatRGatewayService.Length}");
        var methods_MediatRGatewayService = type_MediatRGatewayService.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.DeclaredOnly);
        Console.WriteLine($"[PASS] MediatRGatewayService 公开方法数量: {methods_MediatRGatewayService.Length}");
        foreach (var m in methods_MediatRGatewayService)
            Console.WriteLine($"  [INFO] 方法: {m.Name}");
    }
    else
    {
        Console.WriteLine("[WARN] 类型 MediatRGatewayService 未找到，尝试无命名空间...");
        type_MediatRGatewayService = Type.GetType("MediatRGatewayService");
        if (type_MediatRGatewayService != null)
            Console.WriteLine("[PASS] 类型 MediatRGatewayService (无命名空间) 存在");
        else
            Console.WriteLine("[INFO] 类型 MediatRGatewayService 可能为顶层语句或嵌套类型");
    }
    ...
    // 验证 interface: IMediatRGateway
    var type_IMediatRGateway = Type.GetType("IMediatRGateway");
    if (type_IMediatRGateway != null)
    {
        Console.WriteLine("[PASS] 类型 IMediatRGateway (interface) 存在");
    }
    else
    {
        Console.WriteLine("[WARN] 类型 IMediatRGateway 未找到，尝试无命名空间...");
        type_IMediatRGateway = Type.GetType("IMediatRGateway");
        if (type_IMediatRGateway != null)
            Console.WriteLine("[PASS] 类型 IMediatRGateway (无命名空间) 存在");
        else
            Console.WriteLine("[INFO] 类型 IMediatRGateway 可能为顶层语句或嵌套类型");
    }
