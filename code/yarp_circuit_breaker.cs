...
public sealed class CircuitBreakerMiddleware
{
    ...
    public CircuitBreakerMiddleware(IMemoryCache cache)
    {
        _cache = cache;
    ...
    private async Task ProcessRequestsAsync()
    {
        await foreach (var request in _requestChannel.Reader.ReadAllAsync(_cts.Token))
        {
            var response = await _circuitBreaker.ExecuteAsync(() => 
                ProcessRequestWithBreakerAsync(request));
            request.Completion.SetResult(response);
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    private async Task<ProxyResponse> ProcessRequestWithBreakerAsync(ProxyRequest request)
    {
...
builder.Services.AddStackExchangeRedisCache(options =>
{
    ...
    options.InstanceName = "YarpCircuitBreaker_";
});
...
builder.Services.AddReverseProxy()
    .AddTransforms(context =>
    {
        ...
        {
            var cacheKey = transformContext.ProxyRequest.RequestUri.ToString();
            if (builder.Services.GetRequiredService<IMemoryCache>().TryGetValue(cacheKey, out var cachedResponse))
            {
                transformContext.ProxyResponse = (ProxyResponse)cachedResponse;
                return;
            }
            
            // 添加自定义请求头
            transformContext.ProxyRequest.Headers.Add("X-Forwarded-Time", DateTime.UtcNow.ToString("o"));
        });
        
        // 响应转换
        context.AddResponseTransform(async transformContext =>
        {
            if (transformContext.ProxyResponse.IsSuccessStatusCode)
            {
                var cacheKey = transformContext.ProxyRequest.RequestUri.ToString();
                builder.Services.GetRequiredService<IMemoryCache>().Set(
                    cacheKey, 
                    transformContext.ProxyResponse,
