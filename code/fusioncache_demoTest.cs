#load "fusioncache_demo.cs"

Console.WriteLine("=== fusioncache_demo.cs Test ===");
...
try
{
    ...
    // 验证 class: CachePartitionService
    var type_CachePartitionService = Type.GetType("CachePartitionService");
    if (type_CachePartitionService != null)
    {
        Console.WriteLine("[PASS] 类型 CachePartitionService (class) 存在");
        var ctors_CachePartitionService = type_CachePartitionService.GetConstructors();
        Console.WriteLine($"[PASS] CachePartitionService 构造函数数量: {ctors_CachePartitionService.Length}");
        var methods_CachePartitionService = type_CachePartitionService.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.DeclaredOnly);
        Console.WriteLine($"[PASS] CachePartitionService 公开方法数量: {methods_CachePartitionService.Length}");
        foreach (var m in methods_CachePartitionService)
            Console.WriteLine($"  [INFO] 方法: {m.Name}");
    }
    else
    {
        Console.WriteLine("[WARN] 类型 CachePartitionService 未找到，尝试无命名空间...");
        type_CachePartitionService = Type.GetType("CachePartitionService");
        if (type_CachePartitionService != null)
            Console.WriteLine("[PASS] 类型 CachePartitionService (无命名空间) 存在");
        else
            Console.WriteLine("[INFO] 类型 CachePartitionService 可能为顶层语句或嵌套类型");
    }
