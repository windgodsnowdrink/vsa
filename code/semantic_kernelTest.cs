...
Console.WriteLine("=== semantic_kernel.cs Test ===");
...
try
{
    ...
    // 验证 class: TodoSkill
    var type_TodoSkill = Type.GetType("TodoSkill");
    if (type_TodoSkill != null)
    {
        Console.WriteLine("[PASS] 类型 TodoSkill (class) 存在");
        var ctors_TodoSkill = type_TodoSkill.GetConstructors();
        Console.WriteLine($"[PASS] TodoSkill 构造函数数量: {ctors_TodoSkill.Length}");
        var methods_TodoSkill = type_TodoSkill.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.DeclaredOnly);
        Console.WriteLine($"[PASS] TodoSkill 公开方法数量: {methods_TodoSkill.Length}");
        foreach (var m in methods_TodoSkill)
            Console.WriteLine($"  [INFO] 方法: {m.Name}");
    }
    else
    {
        Console.WriteLine("[WARN] 类型 TodoSkill 未找到，尝试无命名空间...");
        type_TodoSkill = Type.GetType("TodoSkill");
        if (type_TodoSkill != null)
            Console.WriteLine("[PASS] 类型 TodoSkill (无命名空间) 存在");
        else
            Console.WriteLine("[INFO] 类型 TodoSkill 可能为顶层语句或嵌套类型");
    }
