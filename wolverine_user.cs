...
app.MapPost("/user/create", async (UserDto dto,
    IUserService svc, IMessageBus bus) =>
{
    var userId = await svc.CreateAsync(dto.Name);
    await bus.PublishAsync(new UserCreated(userId, dto.Name, DateTimeOffset.UtcNow));
...
public class InMemoryUserRepository : IUserRepository
{
    ...
    {
        var id = Guid.NewGuid();
        _store[id] = new User { Id = id, Name = name };
        return Task.FromResult(id);
    }
    public Task<User> GetAsync(Guid id) => Task.FromResult(_store[id]);
}
...
public class UserService : IUserService
{
    ...
    public Task<Guid> CreateAsync(string name) => _repo.AddAsync(name);
}
...
public class UserHandler
{
    ...
    // 查询用户是否存在（BFF 的 “UserChecked” 步骤会用到）
    public async Task<UserExistsResponse> Handle(UserExistsQuery query)
    {
        var user = await _repo.GetAsync(query.UserId);
        return new UserExistsResponse(query.UserId, user != null);
    }
}