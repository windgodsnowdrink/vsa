...
using SRS.Core;
using SRS.Core.Media;
using FFmpeg.AutoGen;
using Microsoft.Extensions.ObjectPool;

// 1. 高性能流媒体处理器(Disruptor模式)
[SkipLocalsInit]
public sealed class SrsStreamProcessor : IAsyncDisposable
{
    ...
    public SrsStreamProcessor(SRSContext srsContext)
    {
...
// 2. SRS服务器扩展
public static class SrsServiceExtensions
{
    public static IServiceCollection AddSrsServer(
        this IServiceCollection services,
        Action<SrsOptions> configure)
    {
        var options = new SrsOptions();
        configure(options);
        ...
        // 注册SRS上下文
        services.AddSingleton<SRSContext>(sp => 
            new SRSContext(options, sp.GetRequiredService<TieredMemoryServer>()));
            
        // 注册流处理器
        services.AddSingleton<SrsStreamProcessor>();
...
// 配置SRS服务器
builder.Services.AddSrsServer(options =>
{
...
{
    var processor = app.Services.GetRequiredService<SrsStreamProcessor>();
