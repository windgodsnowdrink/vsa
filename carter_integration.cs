...
#:property IlcGenerateCompleteTypeMetadata false
...
// 使用UnmanagedCallersOnly特性标记本地方法
[UnmanagedCallersOnly(EntryPoint = "NativeCallback")]
public static int NativeCallback(int value) => value * 2;

// 在模块启动时预先收集所有需要的类型
[ModuleInitializer]
public static void InitializeAot()
{
    // 使用DynamicDependency确保AOT编译包含所需类型
    DynamicDependency.LoadAssemblyTypes(
        typeof(Program).Assembly,
        typeof(JsonSerializerOptions),
        typeof(HttpClient));
    
    // 注册JSON序列化上下文
    JsonSerializerOptions.Default.TypeInfoResolverChain.Add(
        JsonContext.Default);
}
...
public abstract class CarterModuleBase : CarterModule, ICarterModule
{
    ...
    public static class CarterModuleExtensions
    {
        ...
        // 加载指定目录下的DLL文件
        public static IServiceCollection AddCarterModulesFromDlls(this IServiceCollection services, string directoryPath)
        {
            var dlls = Directory.GetFiles(directoryPath, "*.dll");
            foreach (var dll in dlls)
            {
                var assembly = AssemblyLoadContext.Default.LoadFromAssemblyPath(dll);
                services.AddCarterModules(assembly);
            }
            return services;
        }
    }
...
{
    IEnumerable<ICarterModule> LoadModulesFromDll(string dllPath);
    void WatchForChanges(string directory, Action<ICarterModule> onModuleChanged);
}

// 装饰器模式增强模块系统
public class DecoratedCarterModule : ICarterModule
{
    private readonly ICarterModule _innerModule;
    private readonly ILogger<DecoratedCarterModule> _logger;

    public string ModuleName => _innerModule.ModuleName;

    public DecoratedCarterModule(ICarterModule innerModule, ILogger<DecoratedCarterModule> logger)
    {
        _innerModule = innerModule;
        _logger = logger;
    }
    ...
    public Task InitializeAsync()
    {
        ...
        return _innerModule.InitializeAsync();
    }

    public Task ExecuteAsync(RouteRequest request)
    {
        _logger.LogInformation($"Executing module {ModuleName}");
        return _innerModule.ExecuteAsync(request);
    }
}

// 模块外观类
public class ModuleFacade : IDisposable
{
    private readonly IPluginLoader _pluginLoader;
    private readonly IServiceProvider _serviceProvider;
    private readonly CarterModuleRegistry _registry;
    private readonly List<FileSystemWatcher> _watchers = new();

    public ModuleFacade(IPluginLoader pluginLoader, IServiceProvider serviceProvider)
    {
        _pluginLoader = pluginLoader;
        _serviceProvider = serviceProvider;
        _registry = new CarterModuleRegistry();
    }
    ...
    public void LoadModulesFromDirectory(string directory)
    {
        ...
        {
            var modules = _pluginLoader.LoadModulesFromDll(dll);
        ...
        {
            var module = _pluginLoader.LoadModulesFromDll(e.FullPath).FirstOrDefault();
            if (module != null)
            {
                _registry.RegisterModule(module);
            }
    ...
    public ICarterModule GetModule(string name) => _registry.GetModule(name);
...
public class DefaultPluginLoader : IPluginLoader
{
    ...
    public DefaultPluginLoader(IServiceProvider serviceProvider)
    {
        _serviceProvider = serviceProvider;
    }
    ...
    public IEnumerable<ICarterModule> LoadModulesFromDll(string dllPath)
    {
        ...
        return assembly.GetTypes()
            .Where(t => typeof(ICarterModule).IsAssignableFrom(t) && !t.IsAbstract)
            .Select(t => (ICarterModule)ActivatorUtilities.CreateInstance(_serviceProvider, t));
    }

    public void WatchForChanges(string directory, Action<ICarterModule> onModuleChanged)
    {
        var watcher = new FileSystemWatcher(directory, "*.dll")
        {
            EnableRaisingEvents = true,
        ...
        watcher.Changed += (sender, e) => 
        {
            var module = LoadModulesFromDll(e.FullPath).FirstOrDefault();
            if (module != null)
            {
...
builder.Services.Scan(scan => scan
    .FromAssemblies(AssemblyLoadContext.Default.LoadFromAssemblyPath(dllPath))
...
// 在Program.cs中
builder.Services
    .AddCarterModulesFromDlls("Plugins")
    .Scan(scan => scan
        .FromAssemblies(AssemblyLoadContext.Default.LoadFromAssemblyPath("Plugins/MyService.dll"))
        .AddClasses()
        .AsMatchingInterface());
...
// 在CarterIntegration类中添加AOT优化配置
[assembly: System.Runtime.CompilerServices.DisableRuntimeMarshalling]
[assembly: System.Runtime.Versioning.RequiresPreviewFeatures]

public static class CarterIntegration
{
    // 优化P/Invoke调用
    [LibraryImport("libc", EntryPoint = "rand")]
    [UnmanagedCallersOnly]
    private static partial int rand();

    // 反射优化 - 使用DynamicDependency特性
    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(ProductModule))]
    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CarterModuleBase))]
    
    // 添加Json源生成器支持
    [JsonSerializable(typeof(ProductModule))]
    [JsonSerializable(typeof(CarterModuleBase))]
    private partial class CarterJsonContext : JsonSerializerContext {}
    ...
    public static WebApplication CreateApplication(string[] args)
    {
        ...
        {
            options.SerializerOptions.TypeInfoResolver = CarterJsonContext.Default;
        });
        ...
        if (builder.Environment.IsProduction())
        {
            ...
            builder.Services.AddSingleton<MemoryPool<byte>>(sp => 
                new SlabMemoryPool(SlabMemoryPool.DefaultMaximumBufferSize));
...
{
    public static readonly int DefaultMaximumBufferSize = 1024 * 1024; // 1MB
    
    public SlabMemoryPool(int maximumBufferSize)
    {
    ...
    public override IMemoryOwner<byte> Rent(int minBufferSize = -1)
    {
        ...
        return new SlabMemoryOwner(minBufferSize);
    }
    
    protected override void Dispose(bool disposing) { }
}

internal sealed class SlabMemoryOwner : IMemoryOwner<byte>
{
    private byte[] _array;
    
    public SlabMemoryOwner(int size)
    {
    ...
    public static IApplicationBuilder UseTenantRouting(this IApplicationBuilder app)
    {
        return app.Use(async (context, next) =>
        {
            ...
            context.Items["TenantContext"] = new TenantContext
            {
                ...
                IsValid = !string.IsNullOrEmpty(tenantId)
            };
            await next();
        });
    }
...
// 尾延迟优化器集成
public static class TailLatencyOptimizerExtensions
{
    public static IServiceCollection AddTailLatencyOptimizer(this IServiceCollection services)
    {
...
public static class RateLimitingMiddleware
{
    public static IApplicationBuilder UseRateLimiting(this IApplicationBuilder app)
    {
        ...
        return app.Use(async (context, next) =>
        {
            if (!await rateLimiter.AcquireAsync())
            {
...
public static class CarterIntegration
{
    public static WebApplication CreateApplication(string[] args)
    {
        ...
        // 添加尾延迟优化器
        builder.Services.AddTailLatencyOptimizer();
