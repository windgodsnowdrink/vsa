...
public struct socket_result
{
    ...
    /// 获取异常信息
    /// </summary>
    /// <returns>异常信息字符串，无异常时返回null</returns>
...
/// <summary>
/// 套接字接受丢失处理函数结构体，用于处理套接字接受连接时的丢失情况
/// </summary>
...
public abstract class socket
{
    ...
    /// <param name="bytes">数据指针</param>
    /// <param name="numBytesToWrite">要写入的字节数</param>
    /// <param name="numBytesWritten">实际写入的字节数</param>
    /// <param name="mustBeZero">必须为IntPtr.Zero</param>
    ...
    /// <param name="handle">文件句柄</param>
    /// <param name="bytes">数据指针</param>
    /// <param name="numBytesToRead">要读取的字节数</param>
    /// <param name="numBytesRead">实际读取的字节数</param>
    /// <param name="mustBeZero">必须为IntPtr.Zero</param>
    /// <returns>非零表示成功，零表示失败</returns>
    [DllImport("kernel32.dll")]
    ...
    /// 异步读取文件
    /// </summary>
    /// <param name="buff">字节数组列表</param>
    /// <param name="cb">回调函数</param>
    public void async_reads(IList<byte[]> buff, Action<socket_result> cb)
    {
        _async_reads(0, 0, buff, cb);
    }
    /// <summary>
    /// 异步读取文件
    /// </summary>
    /// <param name="currTotal">当前读取字节数</param>
    /// <param name="currIndex">当前读取索引</param>
    /// <param name="buff">字节数组列表</param>
    /// <param name="cb">回调函数</param>
    ...
    /// <param name="buff">字节数组列表</param>
    /// <param name="cb">回调函数</param>
    public void async_writes(IList<byte[]> buff, Action<socket_result> cb)
    {
        _async_writes(0, 0, buff, cb);
    }
    /// <summary>
    /// 异步写入文件
    /// </summary>
    /// <param name="currTotal">当前写入字节数</param>
    /// <param name="currIndex">当前写入索引</param>
    /// <param name="buff">字节数组列表</param>
    /// <param name="cb">回调函数</param>
...
/// <summary>
/// 套接字TCP实现类，继承自socket抽象基类，提供了TCP套接字的具体实现
/// </summary>
...
/// <summary>
/// 套接字异步操作的同一处理器，适用于同一时间只会有一个异步操作的场景，可以减少每次异步调用时的状态对象分配和回收，提升性能。
/// </summary>
...
#if !NETCORE
/// <summary>
/// 套接字串口设备实现，使用 SerialPort 进行异步读写操作，适用于同一时间只会有一个异步操作的场景，可以减少每次异步调用时的状态对象分配和回收，提升性能。
/// </summary>
...
#endif
/// <summary>
/// 套接字通道抽象类，使用 PipeStream 进行异步读写操作，适用于同一时间只会有一个异步操作的场景，可以减少每次异步调用时的状态对象分配和回收，提升性能。
/// </summary>
