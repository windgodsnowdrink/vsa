...
namespace System.Threading.Tasks.Test
{
    class Program
    {
        ...
        {
            generator.wait_group wg = new generator.wait_group();
            for (int i = 0; i < 2; i++)
            {
                wg.add();
                int idx = i;
        ...
        static async Task Worker17()
        {
            ...
            children.go(async delegate ()
            {
                for (int i = 0; i < 3; i++)
                {
                    var res = await csp.invoke(tuple.make(i, i).value2);
            ...
            children.go(async () =>
            {
                while (true)
                {
                    chan_state st = await generator.csp_wait<int, tuple<int, int>>(csp, async delegate (int p)
                    {
                        Log($"recv {p}");
                        await generator.sleep(1000);
                        return tuple.make<int, int>(p, p);
                    });
