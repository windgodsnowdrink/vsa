...
/// <summary>
/// 系统时钟类，提供高精度的时间戳功能
/// 使用 System.Threading.PeriodicTimer + 自定义高精度计时 ：
/// - 结合 System.Diagnostics.Stopwatch 实现更高精度的计时
/// - 使用 System.Threading.PeriodicTimer 处理异步等待
/// - 使用 Nito.AsyncEx 处理异步任务和定时器
/// - 使用 Channel<T> 作为定时器事件的队列
/// </summary>
...
public class async_timer
{
    ...
    struct steady_timer_handle
    {
        ...
        /// 定时器节点
        /// </summary>
        public Map<long, async_timer> node;
    }
    ...
    internal class steady_timer
    {
        ...
        class waitable_timer
        {
            ...
            /// 工作引擎
            /// </summary>
            work_service.work_engine _workEngine;
            ...
            waitable_timer(bool utcMode)
            {
                ...
                _eventsQueue = new Map<long, waitable_event_handle>();
                _workEngine = new work_service.work_engine();
                _cts = new CancellationTokenSource();
                _timerChannel = Channel.CreateUnbounded<long>();
            ...
            private async Task RunTimerAsync(CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    {
                        // 计算下一次超时时间
                        long currentTime = _utcMode ? utc_tick.get_tick_us() : system_tick.get_tick_us();
                        long nextExpireTime = _expireTime;
                        
                        if (nextExpireTime != long.MaxValue)
                        {
                            long delayUs = nextExpireTime - currentTime;
                            if (delayUs > 0)
                            {
                                // 使用PeriodicTimer进行高精度等待
                                using (var timer = new PeriodicTimer(TimeSpan.FromMicroseconds(delayUs)))
                                {
                        ...
                        // 处理到期的事件
                        _workEngine.service.post(timer_handler);
                    }
                }
            ...
            public void append_event(long absus, waitable_event_handle eventHandle)
            {
                ...
                {
                    if (_exited) return;
                    _eventsQueue.Add(absus, eventHandle);
                    eventHandle.steadyTimer._waitableNode = _eventsQueue;
            ...
            public void remove_event(steady_timer steadyTime)
            {
                _workEngine.service.post(delegate ()
                {
                    ...
                    {
                        long lastAbsus = steadyTime._waitableNode.First().Key;
                        _eventsQueue.Remove(lastAbsus);
            ...
            public void update_event(long absus, waitable_event_handle eventHandle)
            {
                _workEngine.service.post(delegate ()
                {
                    ...
                    {
                        _eventsQueue.Remove(eventHandle.steadyTimer._waitableNode.First().Key);
                        _eventsQueue.Add(absus, eventHandle);
                    }
                    else
                    {
        ...
        /// 可等待节点
        /// </summary>
        Map<long, waitable_event_handle> _waitableNode;
        ...
        public steady_timer(shared_strand strand, bool utcMode)
        {
            ...
            _expireTime = long.MaxValue;
            _strand = strand;
            _timerQueue = new Map<long, async_timer>();
        }
        ...
        public void timeout(async_timer asyncTimer)
        {
            ...
            _timerQueue.Add(absus, asyncTimer);
            asyncTimer._timerHandle.node = _timerQueue;
            if (!_looping)
            {
                _looping = true;
                _expireTime = absus;
        ...
        public void cancel(async_timer asyncTimer)
        {
            ...
            {
                var node = asyncTimer._timerHandle.node.First().Key;
                _timerQueue.Remove(node);
        ...
        public void re_timeout(async_timer asyncTimer)
        {
            ...
            if (null != asyncTimer._timerHandle.node)
            {
                _timerQueue.Remove(asyncTimer._timerHandle.node.First().Key);
                _timerQueue.Add(absus, asyncTimer);
            }
            else
            {
