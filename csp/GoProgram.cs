...
namespace System.Threading.Tasks.Go
{
    class Program
    {
        ...
        static async Task Producer4()
        {
            while (true)
            {
                var res =  _csp.send(system_tick.get_tick_us());
                // var res = await _csp.invoke(system_tick.get_tick_us());
                Console.WriteLine("csp return {0}", res.Result);
                await generator.sleep(1000);
            }
        }
        ...
        static void Main(string[] args)
        {
            ...
            _chan1 = chan<long>.make(3);
            _chan2 = chan<long>.make(0);
            _chan3 = chan<long>.make(-1);
