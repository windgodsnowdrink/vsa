...
// Mediator 源生成器配置（MediatorOptions）：必须只在本文件出现一次（源生成器扫描所有
// Compile 项时会跨文件解析 attribute，重复则抛 "Sequence contains more than one element"）。
global using System;
...
// 必须用 assembly attribute；lambda 形式在某些工具链下不被识别。
